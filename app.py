...
# Key status early, but do NOT create client yet (prevents crashes before UI)
try:
...
# ── OpenAI generator (built only when needed) ────────────────────────────────
def generate_neogen_advert(job_description: str) -> str:
    # Create client lazily here to avoid import/initialization issues blocking UI
...
def to_docx(text: str) -> BytesIO:
    ...
    for line in text.split("\n"):
        doc.add_paragraph(line.strip()) if line.strip() else doc.add_paragraph()
    buf = BytesIO()
    doc.save(buf)
    buf.seek(0)
    return buf

# ── UI: tabs (ensures uploaders render) ──────────────────────────────────────
...
with tab_single:
    ...
    if uploaded:
        try:
            jd_text = extract_text_from_docx(uploaded) if uploaded.name.lower().endswith(".docx") else extract_text_from_pdf(uploaded)
            ...
            if st.button("✨ Generate Advert", key="single_generate"):
                if not OPENAI_API_KEY:
                    ...
                    if advert:
                        ...
                        st.code(advert)  # copy-to-clipboard
...
with tab_batch:
    ...
    st.caption("Tip: drag several files at once. You’ll get a ZIP of adverts.")
        ...
        if not OPENAI_API_KEY:
            ...
            try:
                with ZipFile(zip_buf, "w") as z:
                    for f in files:
                        try:
                            jd_text = extract_text_from_docx(f) if f.name.lower().endswith(".docx") else extract_text_from_pdf(f)
                            if not jd_text:
                                continue
                            advert = generate_neogen_advert(jd_text)
